...
class MasseurController extends MasseurbaseController
{
        ...
        if($order){
            foreach ($order as $key => $value) {
                ...
                if(empty($value['up_time'])){
                    ...
                    $info[$key]['countdown'] = ceil(($info[$key]['down_time']-time())/60);
    ...
    public function up_clock(){
		...
		$order_info=$this->orders->where("id=".$info['order_id'])->find();
    ...
    public function down_clock()
    {
        ...
        if($result){
			...
			*/
			...
			$down_clock_count=$this->ordersproject->where($this->where." and order_id=".$info['order_id']." and down_time=0")->count();
			if($order_info['status']==2 && $down_clock_count==0)
			{
				$this->shopbed->where("id=".$order_info['bed_id'])->save(array('is_lock' => 1));
                $this->shoproom->where('id='.$order_info['room_id'])->save(array('state'=>3,'lock_user_id' => 0));
                $this->shoplockcard->where('id='.$order_info['lockcard_id'])->save(array('is_lock'=>1));
			}
    ...
    public function replace_room()
    {
        ...
        $this->shoproom->where("id=".$order_info['room_id']." and shop_id=".$this->shop_id)->save(array('state' => 3,'lock_user_id' => 0));
        ...
        if($result)
        {
            $this->shoproom->where("id=".$room_id." and shop_id=".$this->shop_id)->save(array('state' => 0,'lock_user_id' => $order_info['shopuser_id']));
    ...
    public function get_project()
    {
        ...
        $where.=" and state=1";
        $project_ids = $this->shopmasseur->where($where.' and id='.$this->masseur_id)->getfield('master_item');
        $map='';
...
//追加信息提交订单
        ...
        if($order_info)
        {
            ...
            if($result)
            {
                ...
                if($shopuser_info['device']!=''){
                    unset($title);
                    unset($content);
					$title="【".$room_name."】包间，【".$masseur_name."】健康师的项目已上钟！";
					$content = "【".$room_name."】包间，【".$masseur_name."】健康师的项目已上钟！";
                    $device[] = $shopuser_info['device'];
    ...
    public function all_down()
    {
        ...
        if($order_info)
        {
			...
			*/
			...
			$down_clock_count=$this->ordersproject->where("shop_id=".$this->shop_id." and order_id=".$order_info['id']." and down_time=0")->count();
			if($order_info['status']==2 && $down_clock_count==0)
			{
				$this->shoplockcard->where("id=".$order_info['lockcard_id']." and shop_id=".$this->shop_id)->save(array("is_lock" => 1));
                $this->shopbed->where("id=".$order_info['bed_id']." and shop_id=".$this->shop_id)->save(array('is_lock' => 1));
            ...
            if($result)
            {
                ...
                if($shopuser_info['device']!=''){
                    unset($title);
                    unset($content);
					$title="【".$room_name."】包间，【".$masseur_name."】健康师的项目已下钟！";
					$content = "【".$room_name."】包间，【".$masseur_name."】健康师的项目已下钟！";
                    $device[] = $shopuser_info['device'];
    ...
    public function index_order(){
        ...
        //获取订单项目
		$one_up_time=$this->ordersproject->where("shop_id=".$this->shop_id." and order_id=".$order_info['id']." and up_time!=0  and masseur_id=".$this->masseur_id)->order("up_time asc")->find();
		$one_down_time=$this->ordersproject->where("shop_id=".$this->shop_id." and order_id=".$order_info['id']." and down_time!=0  and masseur_id=".$this->masseur_id)->order("down_time desc")->find();
        $finish_time=0;
        $project_total_money=0;
        $result_project=$this->ordersproject->where("shop_id=".$this->shop_id." and order_id=".$order_info['id'].' and masseur_id='.$this->masseur_id)->order("down_time asc")->group("masseur_id")->select();
        $order_info['is_confirm'] = $this->ordersproject->where("shop_id=".$this->shop_id." and order_id=".$order_info['id'].' and masseur_id='.$this->masseur_id." and is_confirm=1")->count();
        foreach($result_project as &$val)
        {
            ...
            foreach($val['project_list'] as &$rowtt)
            {
				...
				if($rowtt['up_time'] > 0)
				{
					$rowtt['down_end_data']=date("Y-m-d H:i:s",$rowtt['up_time']+($rowtt['duration']*60));
				}else
				{
					$rowtt['down_end_data']='';
				}
            ...
            $val['down_time_data']=date("Y-m-d H:i:s",$val['down_time']);
            $val['createtime_data']=date("Y-m-d H:i:s",$val['createtime']);
			//$val['down_end_data']=date("Y-m-d H:i:s",$one_up_time['up_time']+($finish_time*60));
			$val['down_end_data_time']=date("Y-m-d H:i:s",$one_down_time['down_time']+(5*60));
            $val['total_price']=$project_total_money;
