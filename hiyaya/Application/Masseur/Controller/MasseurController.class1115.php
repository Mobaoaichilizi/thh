    ...
    public function up_clock(){
        ...
        if($result){
            ...
            if($shopuser_info['device']!=''){
                ...
                $extra = array("push_type" => 1, "order_id" => $info['order_id']);
...
//追加信息提交订单
        ...
        if($order_info)
        {
            ...
            if($result)
            {
                ...
                if($shopuser_info['device']!=''){
                    unset($title);
                    unset($content);
                    $title="订单编号为".$order_info['order_sn']."的".$masseur_name."健康师的项目已全部上钟！";
                    $content = "订单编号为".$order_info['order_sn']."的".$masseur_name."健康师的项目已全部上钟！";
                    $device[] = $shopuser_info['device'];
                    $extra = array("push_type" => 1, "order_id" => $id);
                    $audience='{"alias":'.json_encode($device).'}';
                    $extras=json_encode($extra);
                    $os=$shopuser_info['os'];
                    $res=jpush_shop_send($title,$content,$audience,$os,$extras);
        ...
        if($order_info)
        {
            ...
            if($result)
            {
                ...
                if($shopuser_info['device']!=''){
                    unset($title);
                    unset($content);
                    $title="订单编号为".$order_info['order_sn']."的".$masseur_name."健康师的项目已全部下钟！";
                    $content = "订单编号为".$order_info['order_sn']."的".$masseur_name."健康师的项目已全部下钟！";
                    $device[] = $shopuser_info['device'];
                    $extra = array("push_type" => 1, "order_id" => $id);
                    $audience='{"alias":'.json_encode($device).'}';
                    $extras=json_encode($extra);
    ...
    public function index_order(){
        $this->where.=" and ((up_time=0) or (down_time=0) or (pay_time=0))";
        $orderproject = $this->ordersproject->where($this->where.' and masseur_id='.$this->masseur_id.' and status=1  and is_del=0')->order('createtime desc,id desc')->find();
