   ...
   function _initialize(){
		...
		if(!empty($session_admin_id))
		{
			...
			if($session_admin_id == 1){
				...
				$menu_list= $node->where("parentid=0 and status=1 and id in (select node_id from dade_access where role_id=".$admin_user['role_id'].")")->order('listorder')->select(); 
			   foreach($menu_list as $n=> $val){	      
				  $menu_list[$n]['voo']=$node->where("parentid=".$val['id']." and id in (select node_id from dade_access where role_id=".$admin_user['role_id'].")")->order('listorder')->select();  
				  foreach($menu_list[$n]['voo'] as $k=> $row)
				 {
					$menu_list[$n]['voo'][$k]['ttt']=$node->where("parentid=".$row['id']." and id in (select node_id from dade_access where role_id=".$admin_user['role_id'].")")->order('listorder')->select();  
				 }		
			   }
			}
