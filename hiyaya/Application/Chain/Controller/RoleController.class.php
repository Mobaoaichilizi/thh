...
class RoleController extends ChainbaseController {
	protected $role,$menu,$access,$user;
	
	public function _initialize() {
		parent::_initialize();
		$this->role = D("ChainRole");
		$this->menu = D("ChainMenu");
		$this->access = D("ChainAccess");
		$this->user = D("ChainUser");
	...
	public function saveAccess()
	{
			...
			if (is_array($_POST['menuid']) && count($_POST['menuid'])>0) {
    			...
    			foreach ($_POST['menuid'] as $menuid) {
    				...
    				if($menu){
    					...
    					$this->access->add(array("role_id"=>$role_id,"node_name"=>$name,'node_id'=>$menu['id'],'chain_id'=>$chain_id));
    				}
    			}
    			...
    			//当没有数据时，清除当前角色授权
    			$this->access->where(array("role_id" => $role_id,'chain_id'=>$this->chainid))->delete();
		...
		}else
		{
			...
			$priv_data=$this->access->where(array("role_id"=>$role_id,'chain_id'=>$this->chainid))->getField("node_id",true);//获取权限表数据
