...
class OrderController extends ApibaseController {
    ...
    public function index()
	{
		...
		$use_room_count=$this->shoproom->where("shop_id=".$this->shopid." and state=0 and lock_user_id=".$this->userid)->count();
	...
	public function replace_room()
	{
		...
		*/
		...
		$bed_info=$this->shopbed->where("room_id=".$room_id." and is_lock=1")->order("sort asc")->find();
		if(!$bed_info)
		{
			unset($data);
			$data['code']=1;
		...
		{
			$this->shoproom->where("id=".$room_id." and shop_id=".$this->shopid)->save(array('state' => 0,'lock_user_id' => $this->userid));
			$this->shopbed->where("id=".$bed_info['id']." and shop_id=".$this->shopid)->save(array('is_lock' => 0));
			unset($data);
			$data['code']=0;
			$data['msg']='更新成功！';
			outJson($data);
