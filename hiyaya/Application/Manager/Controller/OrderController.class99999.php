...
class OrderController extends ManagerbaseController {
	...
	public function post_shopproject()
	{
		...
		$result=$this->shopmasseur->field("id,masseur_sn,nick_name,category_id,sex")->where("shop_id=".$this->shopid." and id in (".$masseur_id.")")->select();
		$total_price=0;
		foreach($result as &$val)
		{
			...
			$val['price']=$project_price+$level_proportion;
			...
			$val['p_types']=1;
			$total_price+=$project_price+$level_proportion;
		}
		unset($data);
		$data['state']='success';
		$data['info']='添加成功！';
	...
	public function post_zadd_shopproject()
	{
		...
		$result=$this->shopmasseur->field("id,masseur_sn,nick_name,category_id,sex")->where("shop_id=".$this->shopid." and id=".$masseur_id)->select();
		$total_price=0;
		foreach($result as &$val)
		{
			$level_proportion=$this->masseurcategory->where("id=".$val['category_id'])->getField("level_proportion");
			...
			$val['price']=$project_info['item_price']+$level_proportion;
			$val['duration']=$project_info['item_duration'];
			$val['p_types']=1;
			$total_price+=$project_info['item_price']+$level_proportion;
		}
		unset($data);
		$data['state']='success';
		$data['info']='添加成功！';
	...
	public function post_orders()
	{
		...
		$is_room=$this->shoproom->where("id=".$bed_id." and shop_id=".$this->shopid." and state=0")->find();
		if($is_room)
		{
			$this->error("包间已经被占用！");
		}
		...
		if($result)
		{
			foreach($p_id as $key=>$val)
			{
				if($p_types[$key]==1)
				{
					...
					$is_up_lock=$this->shopscheduling->where("shop_id=".$this->shopid." and masseur_id=".$masseur_id[$key]." and start_time < ".time()." and end_time > ".time()." and is_lock=0")->find();
					if($is_up_lock)
					{
					...
					$masseur_info_level=$this->shopmasseur->where("id=".$masseur_id[$key])->find();
					...
					$project_data=array(
						...
						'number' => $number[$key],
						'total_price' => ($number[$key]*$project_info['item_price'])+($number[$key]*$level_proportion),
					...
					$pay_total_money+=($number[$key]*$project_info['item_price'])+($number[$key]*$level_proportion);
	...
	public function show_order()
	{
		...
		$order_info['shopuser_name']=$this->shopuser->where("id=".$order_info['shopuser_id'])->getField("name");
	...
	public function all_down()
	{
		...
		if($order_info)
		{
			...
			{
				$this->shopscheduling->where("shop_id=".$this->shopid." and masseur_id=".$val['masseur_id']." and id=".$val['scheduling_id'])->save(array('is_lock' => 1));
				if($val['types']==1 || $val['types']==4)
				{
					$this->shopscheduling->where("shop_id=".$this->shopid." and masseur_id=".$val['masseur_id']." and id=".$val['scheduling_id'])->save(array('is_rounds' => 0));
				}
			}
	...
	public function one_down_clock()
	{
		...
		if($order_project_info)
		{
			...
			if($order_project_info['types']==1 || $order_project_info['types']==4)
			{
				$this->shopscheduling->where("shop_id=".$this->shopid." and masseur_id=".$order_project_info['masseur_id']." and id=".$order_project_info['scheduling_id'])->save(array('is_rounds' => 0));
			}
			...
			foreach($p_id as $key=>$val)
			{
				if($p_types[$key]==1)
				{
					...
					*/
					...
					$orderproject_info = $this->ordersproject->where("shop_id=".$this->shopid." and order_id=".$order_id." and masseur_id=".$masseur_id[$key])->find();
					
					$orderproject_info_count=$this->ordersproject->where("shop_id=".$this->shopid." and order_id=".$order_id." and masseur_id=".$masseur_id[$key])->count();
					...
					if($masseur_info_level['device']!='')
					{
						...
						$content = "您有一个新的订单需要处理！";
						$device[] = $masseur_info_level['device'];
						$extra = array("push_type" => 1, "order_id" => $order_info['id'],'is_confirm' => $orderproject_info_count);
						$audience='{"alias":'.json_encode($device).'}';
						$extras=json_encode($extra);
						$os=$masseur_info_level['os'];
						$res=jpush_send($title,$content,$audience,$os,$extras);
	...
	public function get_order_lists()
	{
		...
		foreach($result as &$val)
		{
			...
			$val['pay_tpyes']=get_pay_typename($val['pay_type']);
		}
	...
	public function print_listing()
	{
		...
		{
			$shop_info=$this->shop->where("id=".$this->shopid)->find();
			$shop_user=$this->shopuser->where("id=".$order_info['shopuser_id'])->find();
			...
			if($order_info['member_id']!=0)
			{
				...
				$member_info['numcard']=$this->shopnumcard->where("member_id=".$member_info['id']." and chain_id=".$this->chainid." and card_num > 0")->select();
				//$member_info['coursecard']=$this->shopcoursecard->where("member_id=".$member_info['id']." and shop_id=".$this->shopid)->select();
				
				$member_info['coursecardproject']=$this->shopcourseproject->where("member_id=".$member_info['id']." and chain_id=".$this->chainid." and card_num > 0")->select();
				
				$member_info['deadlinecard']=$this->shopdeadlinecard->where("member_id=".$member_info['id']." and chain_id=".$this->chainid." and start_time < ".time()." and end_time > ".time())->select();
	...
	public function get_order_addmasseura()
	{
		...
		*/
		...
		$masseur_name = I('request.name');
		$is_level = I('request.is_level',$masseur_level_info['id'],'intval');
		$is_types = I('request.is_types',2,'intval');
		$is_sex = I('request.is_sex','男');
		if($masseur_name){
			$where.=" and b.masseur_name like '%$masseur_name%'";
		}
		if($is_level){
		...
		//echo $this->shopscheduling->getLastSql();
		$number=0;
		$number_list=$this->shopscheduling->where("start_time < ".time()." and end_time > ".time()." and shop_id=".$this->shopid." and (status=1 or status=2 or status=3)")->group("status")->select();
		...
		if($number > 1)
		{
			$number_info=$this->shopscheduling->where("start_time < ".time()." and end_time > ".time()." and shop_id=".$this->shopid." and (status=1 or status=2 or status=3)")->group("status")->order("end_time desc")->find();
			//print_r($number_info);
			$is_show=$this->roundlog->where("shop_id=".$this->shopid." and types=1 and createtime >=".$number_info['start_time'])->find();
			if(!$is_show)
			{
				unset($data_array);
				$data_array=array(
				...
				$this->roundlog->add($data_array);
				$show_info=$this->shopscheduling->where("start_time < ".time()." and end_time > ".time()." and shop_id=".$this->shopid." and (status=1 or status=2 or status=3)")->group("status")->order("end_time desc")->find();
				$show_list=$this->shopscheduling->where("start_time =".$show_info['start_time']." and end_time = ".$show_info['end_time']." and shop_id=".$this->shopid." and status=".$show_info['status'])->order("end_time desc")->select();
				foreach($show_list as $rowt)
				{
					$this->shopscheduling->where("id=".$rowt['id'])->save(array('is_rounds' => 1));
				}
			}
		}
		//print_r($number);
		
		$result=$this->shopscheduling->table("dade_shop_scheduling as a")->join("left join dade_shop_masseur as b on a.masseur_id=b.id")->where("a.start_time < ".time()." and a.end_time > ".time()." and b.state=1 and a.shop_id=".$this->shopid." and a.is_lock=1 and FIND_IN_SET(".$shop_project_info['id'].",b.master_item) ".$where)->field("a.masseur_id as masseur_id,a.status as status,b.masseur_sn as masseur_sn,b.masseur_name as masseur_name,b.nick_name as nick_name,b.category_id as category_id,b.sex as sex,a.is_rounds as is_rounds")->order("a.id asc")->select();
		...
		foreach($result as $key=>&$val)
		{
			...
			$arr1[$key]  = $val['down_time'];
			$is_rounds[$key] = $val['is_rounds'];		
		}
		//dump($result);
		//echo "22322711";
		...
		//$arr1=array_column($result,'down_time');
		//print_r($arr1);
		//$is_rounds = array_column($result,'is_rounds');
		//print_r($is_rounds);
		//array_multisort($is_rounds,SORT_DESC,SORT_NUMERIC,$result);
		//array_multisort($arr1,SORT_ASC,SORT_NUMERIC,$result);
		//array_multisort($is_rounds,SORT_DESC,SORT_NUMERIC,$result);
		array_multisort( $is_rounds, SORT_DESC, $arr1, SORT_ASC,$result);
		if($is_types==1)
		{
			$result=array_slice($result,0,$shop_project_info['masseur_num']);
		}
