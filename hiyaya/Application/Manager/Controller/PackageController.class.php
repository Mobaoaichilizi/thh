...
class PackageController extends ManagerbaseController
{
    ...
    public function json()
    {
        ...
        $list    = $this->ShopPackage->where($this->where)->order('id')->page($pagecur.','.$pagenum)->select();
    ...
    public function open()
    {
        ...
        $data=array("state"=>1);
        $res=$this->ShopPackage->where('id='.$id)->save($data);
        if($res)
        {
            echo 'success';
        }
    ...
    {
        $id = I('post.id',0,'intval');
        $res=$this->ShopPackage->where("id=".$id)->find();
        if($res['state']==1){
            $this->error("启用中的会员卡套餐禁止删除！");
        }
        if ($this->ShopPackage->delete($id)!==false)
        {
            $this->success("删除成功");
        }
        else
	...
	public function add()
    {
       if($_POST['act']=='add')
       {
           ...
           if($this->ShopPackage->create()!==false)
           {
               if ($this->ShopPackage->add($data))
               {
                   $this->success("保存成功");
               }
               else
           ...
           {
               $this->error($this->ShopPackage->getError());exit;
           }
       }
    ...
    public function edit()
    {
        if($_POST['act']=='update')
        {
            ...
            if (unique($this->where,'package_name',$data['package_name'],$this->ShopPackage)== false)
            {
                $this->error("套餐名称已经存在！");
                exit;
            }
        ...
        if($_GET['id'])
        {
            ...
            $info= $this->ShopPackage->where('id='.$_GET['id'])->find();
