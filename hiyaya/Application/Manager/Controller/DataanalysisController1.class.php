...
class Dataanalysis1Controller extends ManagerbaseController {
...
//        $pagecur = !empty($pagecur)? $pagecur:1;
	...
	public function individual(){
		...
		//轮钟数
		$loop_clock = $this->OrdersProject->where("shop_id=".$this->shopid." and createtime>".$start_time." and createtime<".$end_time." and is_del=0 and order_id in(".$orders_ids.") and types=1")->count();
		$this->assign("loop_clock",$loop_clock);
		//点钟数
		$point_clock = $this->OrdersProject->where("shop_id=".$this->shopid." and createtime>".$start_time." and createtime<".$end_time." and is_del=0 and order_id in(".$orders_ids.") and types=2")->count();
		$this->assign("point_clock",$point_clock);
		//加钟数
		$add_clock = $this->OrdersProject->where("shop_id=".$this->shopid." and createtime>".$start_time." and createtime<".$end_time." and is_del=0 and order_id in(".$orders_ids.") and types=3")->count();
		$this->assign("add_clock",$add_clock);
		$redata=array();
		$numberdata=array();
		for($i=0;$i<$poor_day+1;$i++)
		{	
			...
			//加钟数
			unset($add_clock_res);
			$add_clock_res = $this->OrdersProject->where("shop_id=".$this->shopid." and ".strtotime($redata[$i]).' < createtime  and createtime <'.(strtotime($redata[$i])+24*3600-1)." and is_del=0 and order_id in(".$orders_ids.") and types=2")->count();
		...
		$this->assign("loop_click_num",array_reverse($loop_click_num));
		$this->assign("point_click_num",array_reverse($point_click_num));
		$this->assign("add_click_num",array_reverse($add_click_num));
