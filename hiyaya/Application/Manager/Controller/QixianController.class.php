...
class QixianController extends ManagerbaseController
{
    ...
    public function json()
    {
        ...
        $list    = $this->ShopQixian->where($this->where)->order('id')->page($pagecur.','.$pagenum)->select();
...
//            }
        ...
        $count   = $this->ShopQixian->where($this->where)->count();
    ...
    public function open()
    {
        ...
        $data=array("state"=>1);
        $res=$this->ShopQixian->where('id='.$id)->save($data);
        if($res)
        {
            echo 'success';
        }
    ...
    {
        $id = I('post.id',0,'intval');
        $res=$this->ShopQixian->where("id=".$id)->find();
        if($res['state']==1){
            $this->error("启用中的会员卡套餐禁止删除！");
        }
        if ($this->ShopQixian->delete($id)!==false)
        {
            $this->success("删除成功");
        }
        else
	...
	public function add()
    {
       if($_POST['act']=='add')
       {
           ...
           $data=array(
               ...
               'expiry_during'=>I('post.expiry_during'),
               'limit_times'=>I('post.limit_times'),
           ...
           if($this->ShopQixian->create()!==false)
           {
               if ($this->ShopQixian->add($data))
               {
                   $this->success("保存成功");
               }
               else
           ...
           {
               $this->error($this->ShopQixian->getError());exit;
           }
       }
    ...
    public function edit()
    {
        if($_POST['act']=='update')
        {
            ...
            if (unique($this->where,'package_name',$data['package_name'],$this->ShopQixian)== false)
            {
                $this->error("套餐名称已经存在！");
                exit;
            }
        ...
        if($_GET['id'])
        {
            ...
            $info= $this->ShopQixian->where('id='.$_GET['id'])->find();
