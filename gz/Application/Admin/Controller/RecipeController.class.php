...
class RecipeController extends AdminbaseController {
	...
	{
		$admin_id=session('admin_id');
		$is_pharmacy = $this->admin->where("id=".$admin_id)->getField("is_pharmacy");
		if($is_pharmacy){
		...
		$admin_count = $this->admin->where("id=".$admin_id." and is_pharmacy = 1")->count();
		if($admin_count){
			$yun_free = $this->admin->where("id=".$admin_id)->getField('yun_free');
		}else{
		...
		if(IS_POST){
			...
			$hrebs_total = $price;
			$pre_type = $this->pre->where("id=".$id)->getField("setting_id_class");
			if($status < 75 ){
				if($is_show == 2){
					$status = 78;
				}else if($is_show == 1){
		...
		$results = $this->pre->field("setting_id_class,setting_id_type,hrebs_id,patient_id,doctor_id,img_thumb_common,img_thumb_prime,pre_description,pre_bz,status,admin_pre,address_id,create_time,is_show,yun_free,dosage_name,total_price,process_price")->where("id = '".$id."'")->find();
		...
		$result['process_price'] = $results['process_price'];
		$result['id'] = $id;
		...
		if(!empty($results['hrebs_id']) || ($results['hrebs_id']!=0)){
			...
			if(!empty($hrebs['pre_hrebs_id'])){
				...
				if($result['dosage_name'] == 131){
					...
					$jg = $jgf['paste_price']*$result['hrebs_number'];
				}
			}
	...
	public function addHrebs(){
		$pre_id = I('get.id',0,'intval');
 		$dosage_name = I('get.dosage_name','131');
 		...
 		if(IS_POST){
 			...
 			$hrebs_number = I('post.hrebs_number',0,'intval');
			...
			$where.=" and status=1";
	 		...
	 		$this->assign('hrebs_number',$hrebs_number);
 	...
 	public function getQuilty(){
 		$admin_id=session('admin_id');
			$admin_count = $this->admin->where("id=".$admin_id." and is_pharmacy = 1")->count();
			$admin_default = $this->admin->where("is_default=1")->getfield("id");
			$where = '';
			if($admin_count > 0){
				$where.=" and admin_id=".$admin_id;
			}else{
				$where.=" and admin_id=".$admin_default;
			}
  ...
  public function pre_operate()
	{
		...
		if($is_pharmacy){
			...
			$rate = $this->admin->where("is_default=1")->getField('rate');
			$platform = $this->admin->where("is_default=1")->getField('platform');
		}
		...
		if($this->pre->save($dataarray)!==false) {
			if($status == "77"){
				...
				financial_log($doctor_id,$bala,3,$ba,'处方收益',8);
				//平台分佣
