...
public function patient_diaed(){
	...
	$p=($p-1)*$limit;
		...
		$ress['patient'] = $this->patient->join("lgq_graphic on lgq_graphic.patientmember_id = lgq_patientmember.user_id")->field("user_id,lgq_patientmember.img_thumb,lgq_patientmember.name,lgq_patientmember.phone,lgq_graphic.id,type,create_time")->where($map0)->select();
		foreach($ress as $k=>$v){
			foreach ($v as $key => $value) {
				$ress['patient'][$key]['create_time'] = format_date($this->graphic->where($map0)->getfield('create_time'));
			}	
