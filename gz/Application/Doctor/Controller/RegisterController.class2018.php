...
public function do_register(){
		...
		$res=array(
			...
			'share_number' => get_rand_number(100000000,999999999,1),
			'member_level ' => 0,
			'os' => $os,
		    'deviceid' => $deviceid,
		    'is_login' => 1,
			...
			{	
				if(!empty($inviter_share_number)){
					...
					if($inviter_person['role'] == 1){
						setpoints($results,1,5,'元','151',0);
						$balance = $this->user->where('id='.$results)->getField('balance');
						financial_log($results,5,3,$balance,'被邀请注册',8);
						...
						setpoints($results,1,100,'分','151',0);
						$score = $this->user->where('id='.$results)->getField('score');
						financial_log($results,100,3,$score,'被邀请注册',8,2);
					...
					setpoints($results,1,3,'元','新用户注册',0);
					$balance = $this->user->where('id='.$results)->getField('balance');
					financial_log($results,3,3,$balance,'新用户注册',8);
				}
			unset($data);
			$data['code']=1;
			$data['message']="注册成功！";
...
public	function register_phone()
	{	
		...
		$res=$this->sms->where("phone='".$username."'")->find();
		if(!empty($res))
		{
			unset($data);
			$data['code']=0; 
