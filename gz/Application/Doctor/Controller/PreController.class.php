...
public function upload_elepre(){
	...
	$dosage_name = I('post.dosage_name');
 	...
 	if(!empty($admin_pre)){
 		$yun_free = $this->admin->where("id=".$admin_pre)->getfield("yun_free");
 	}
	...
	if($dosage_name == 131){
		$jgf = $this->admin->where("id=".$admin_pre)->getfield('technical_price');
	}else if($dosage_name == 132){
		$jgf = $this->admin->where("id=".$admin_pre)->getfield('frying_price');
	}else if($dosage_name == 133){
		$jgf = $this->admin->where("id=".$admin_pre)->getfield('paste_price');
	}
	$process_price = $jgf*$hrebs_number;
	unset($res);
	$res = array(
		'setting_id_class' => 43,
	...
	$re['pre_id'] = $results;
	if($re){
		$patient_info = $this->user->where("id=".$patient_id." and role=2")->find();
		if($patient_info['deviceid']!='')
		{
			$title="";
			$content = "亲,您有一个新的处方订单！";
			$device[] = $patient_info['deviceid'];
			$extra = array("type" => "6", "type_id" => $results,'user_type' => 1,'status' => 72);
...
public function pre_list(){
	...
	$arr2 = array_map(create_function('$n', 'return $n["create_time"];'), $results);
	array_multisort($arr2,SORT_DESC,SORT_NUMERIC,$results);
	$electron = array();
	foreach ($result as $k=>$vo){
		 	$id=intval($vo['patient_id']);
		 	$electron[$id]=isset($electron[$id])?$electron[$id] : $vo;
	...
	foreach ($results as $k=>$vo) {
		$id=intval($vo['patient_id']);
		 	$tradition[$id]=isset($tradition[$id])?$tradition[$id] : $vo;
	}
...
public function hrebs_list()
	{
		...
		$result = $this->single_hrebs->field("hrebs_name,unit_price,id,setting_id_model")->where("firstletter='".$firstletter."' and setting_id_model=".$setting_id_type." and admin_id=".$admin_id." and status=1")->select();
		if($result){
			unset($data);
			$data['code'] = 1;
			$data['message'] = "药材加载成功！";
	...
	public function yaopu(){
		...
		$pha = $this->admin->field('id,admin_name,rate')->where('is_pharmacy=1')->order('sort asc')->select();//查询出所有的药堂
	...
	public function usage(){
		$result = $this->setting->field('id,title')->where('parentid=56 and status=1')->order('sort asc')->select();
		if($result){
			unset($data);
			$data['code'] = 1;
			$data['message'] = "success";
		...
		if($this->patientmember->add($res)){
			...
			if($results){
			  ...
			  //    	);
 				...
 				// 	M('User')->where("id=".$patient_id)->setInc('balance',"10");
					// // setpoints($patient_id,2,5,'元','151',0); 
					// $balance = $this->user->where('id='.$patient_id)->getField('balance');
					// financial_log($patient_id,10,3,$balance,'被邀请注册',8);
					// setpoints($id,1,10,'元','144',0);
					// $ba = $this->user->where('id='.$id)->getField('balance');
					// financial_log($id,10,3,$ba,'邀请注册',8);
					// //
					// $invite_num = $this->user->where('inviter_share_number='.$inviter_share_number)->count();
					// if($invite_num == 10){
					// 	setpoints($id,1,22,'元','153',0);
					// 	$money = $this->user->where('id='.$id)->getField('balance');
					// 	financial_log($id,22,3,$money,'邀请累计十人',8);
					// }else if($invite_num == 5){
					// 	setpoints($id,1,10,'元','152',0);
					// 	$money = $this->user->where('id='.$id)->getField('balance');
					// 	financial_log($id,10,3,$money,'邀请累计五人',8);
					// }
					//
					$patient_information = $this->patientmember->field('user_id,name,img_thumb,sex,age')->where("user_id=".$patient_id)->find();
