...
public function exit_pay(){
	    ...
	    if($status == 1 && $is_replay == 0){
	    	if($uid == $patient){
				...
				{
					$title="";
					$content = "亲,您有图文咨询被取消，请注意查看！";
					$device[] = $uid_phone['deviceid'];
					// $device[] = '866985035736805';
					$extra = array("type" => "2", "type_id" => $id,'user_type' => 1,'status' => 5);
				...
				if($uid_phone['deviceid']!='')
				{
					...
					$content = "亲,您有图文咨询被取消，请注意查看！";
					$device[] = $uid_phone['deviceid'];
					$extra = array("type" => "2", "type_id" => $id,'user_type' => 2,'status' => 4);
					$audience='{"alias":'.json_encode($device).'}';
					$extras=json_encode($extra);
					$os=$uid_phone['os'];
	...
	public function exit_consult()
	{
		...
		financial_log($doctor_id,$money,3,$balance,'图文咨询收益',1);
		...
		if($result){
			...
			if($uid_phone['deviceid']!='')
			{
				...
				$content = "亲,您有图文咨询结束！";
				$device[] = $uid_phone['deviceid'];
				$extra = array("type" => "2", "type_id" => $graphic_id,'user_type' => $type_role,'status' => 6,'doctor_id'=>md5(md5($doctor_id)));
				$audience='{"alias":'.json_encode($device).'}';
				$extras=json_encode($extra);
				$os=$uid_phone['os'];
				jpush_send($title,$content,$audience,$os,$extras);
	...
	public function visits(){
		...
		$patient_id =  $this->user->where("md5(md5(id))='".$patient_id."'")->getfield("id");
		$uid = $this->uid;
		$res = array(
			'status' => 2,
			'is_replay' => 1,
		);
		$info = $this->graphic->where("patientmember_id=".$patient_id." and member_id=".$uid." and status=1")->count();
		if($info == 1){
			$id = $this->graphic->where("patientmember_id=".$patient_id." and member_id=".$uid." and status=1")->getField("id");
			$result = $this->graphic->where('id='.$id)->save($res);
			if($result){
				unset($data);
				$data['code'] = 1;
	...
	/*根据医生和用户的加密id查询最新一条图文咨询的详情*/
	public function last_consult(){
		$uid = $this->uid;//医生的ID
		$patient_id = I('post.patient_id');
		// $patient_id = '7a5eb5b4b7034c6780707d9a9a568136';
		$patient_id = $this->user->where("md5(md5(id))='".$patient_id."'")->getfield("id");
		$result = $this->graphic->where("patientmember_id=".$patient_id." and member_id=".$uid." and status in (1,2)")->order("create_time desc")->limit(1)->find();
		$patient = $this->patient->field("user_id as patient_id,name,img_thumb,phone")->where("user_id='".$result['patientmember_id']."'")->find();
		$results['doctor_phone'] = $this->user->where('id='.$uid)->getfield('username');
		
		$results['id'] = $result['id'];
		$results['doctor_id'] = $uid;
		$results['name'] = $patient['name'];
		$results['patient_id'] = $patient['patient_id'];
		$results['img_thumb'] = $patient['img_thumb'];
		$results['phone'] = $this->user->where("id=".$patient['patient_id'])->getfield("username");
		$results['patient_uid'] = md5(md5($patient['user_id']));
		// $results['phone'] = str_pad_star($patient['phone'],7,'****','left');
		$results['description'] = $result['description'];
		$results['img_desc'] = $result['img_thumb'];
		$results['status'] = $result['status'];
		$results['price'] = $result['money'];
		if($results){
			unset($data);
