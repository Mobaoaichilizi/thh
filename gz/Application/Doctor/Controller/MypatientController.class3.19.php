...
public function patient_diaed(){
	...
	$p=($p-1)*$limit;
		...
		$ress['patient'] = $this->patient->join("lgq_reserve on lgq_reserve.patientmember_id = lgq_patientmember.user_id")->field("user_id,img_thumb,lgq_patientmember.name,lgq_reserve.id,type,res_date,create_time")->where($map2)->select();
		foreach($ress as $k=>$v){
			foreach ($v as $key => $value) {
				$ress['patient'][$key]['phone'] = $this->user->where('id='.$ress['patient'][$key]['user_id'])->getfield('username');
				$ress['patient'][$key]['patient_id'] = md5(md5($ress['patient'][$key]['user_id']));
