...
public function form_collect(){
		...
		foreach ($results as $k => $value) {
			...
			$result[$k]['item_thumb']=explode(',',$result[$k]['item_thumb']);
			$result[$k]['name'] = $this->member->where("user_id='".$result[$k]['uid']."'")->getField('name');
			$result[$k]['img_thumb'] = $this->member->where("user_id='".$result[$k]['uid']."'")->getField('img_thumb');
