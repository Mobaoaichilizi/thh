...
public function do_register(){
		...
		if($results)
		{
			...
			{	
				...
				// 		setpoints($result,1,50,'元','151',0);
				// 		$balance = $this->user->where('id='.$results)->getField('balance');
				// 		financial_log($result,50,3,$balance,'被邀请注册',8);
				// 		setpoints($inviter_person['id'],1,50,'元','144',0);
				// 		$ba = $this->user->where('id='.$inviter_person['id'])->getField('balance');
				// 		financial_log($inviter_person['id'],50,3,$ba,'邀请注册',8);
				// 		if($invite_num == 10){
				...
				// 		setpoints($result,1,10,'元','151',0);
				// 		$score = $this->user->where('id='.$results)->getField('balance');
				// 		financial_log($result,10,3,$score,'被邀请注册',8);
				// 		setpoints($inviter_person['id'],2,10,'元','144',0);
				// 		$score1 = $this->user->where('id='.$inviter_person['id'])->getField('balance');
				// 		financial_log($inviter_person['id'],10,3,$score1,'邀请注册',8);
				// 		if($invite_num == 10){
				...
				// }else{
				// 	setpoints($result,1,50,'元','154',0);
				// 	$balance = $this->user->where('id='.$result)->getField('balance');
				// 	financial_log($result,50,3,$balance,'新用户注册',8);
				// }
			$res = array(
				"doctor_id"=>$result,
				'createtime' => time(),
			...
			unset($results);
			$results = $this->member->field("name,sex,age,status,img_thumb")->where("user_id ='".$result."'")->find();
			$results['username'] = $username;
			$results['uid'] = md5(md5($result));
			unset($data);
			$data['code']=1;
			$data['message']="注册成功！";
			$data['info']=$results;
...
public	function register_phone()
	...
	public function agreement(){
		$result = $this->information->where('setting_id=218')->find();
		if($result){
