...
public function published()
	{
		...
		// $type = 1;//论坛类型 1：图文 2：视频
		$title = I('post.title');
		// $title = '111';
		$content = $_POST['content'];

		$setting_id = I('post.setting_id','26');
		// $setting_id = '23';
...
public function forum_details()
	...
	public function do_praise()
	{
		...
		// $uid = $this->uid;
		$id = I('post.id');//论坛ID
		$type = I('post.type');//点赞类型 1：点赞 2：收藏 3: 分享
		if(empty($id)){
			unset($data);
			$data['code'] = 0;
			$data['message'] = "此信息不存在！";
	...
	public function shareForum(){
		...
		if(empty($id)){
			unset($data);
			$data['code'] = 0;
			$data['message'] = "没有所选的论坛！";
			outJson($data);
		}
		$result = $this->forum->field('id,title,item_thumb,video,content,status')->where('id='.$id)->find();
		if($result['item_thumb'] != ''){
			$results = explode(',', $result['item_thumb']);
			$img_thumb = $results[0];
		}else{
			$img_thumb = 'http://'.$_SERVER['HTTP_HOST'].'/Public/Home/images/logo_tit.png';
		}
		$res = array(
			'title' => $result['title'],
			'img_thumb' => $img_thumb,
			'content' => $result['content'],
			'url' => 'http://'.$_SERVER['HTTP_HOST'].'/index.php/Home/forum/share/id/'.$result['id'].'.html',
		);
		unset($data);
		$data['code'] = 1;
		$data['message'] = "成功！";
