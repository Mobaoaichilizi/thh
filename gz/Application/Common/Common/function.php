...
function get_week_date()
{
    ...
    foreach($data as $k=>$v)
    {
		...
		$arr[$k]['year'] = date('Y',strtotime($v));
		if(strtotime($v)+12*3600 <= time()){
        	$arr[$k]['status'] = 0;
        }else{
        	$arr[$k]['status'] = 1;
        }
...
//积分明细
function setpoints($uid,$role,$score,$content,$txt,$opid=0){
     $data=array(
           'user_id'    => $uid, //会员ID，就你要给那个会员操作积分就传入那个会员的ID
           'role'   => $role,
           'score'  => $score,//操作的积分数量,正数为加分，负数为减分；
           'content' => $content,
           'setting_id'  => $txt,   //操作理由，简单的积分操作理由；
           'optime' => time(), //操作时间
           'opid'   => $opid  //操作员ID,如果为0表示系统操作；
     );
     M('ScoreList')->add($data);    //写入积分操作明细数据；
     if($role == 1){
     	 M('User')->where("id=".$uid)->setInc('balance',$score);   //更新会员表余额字段；
     }else if($role == 2){
     	M('User')->where("id=".$uid)->setInc('score',$score);   //更新会员表积分字段；
     }
...
//收支明细
function financial_log($uid,$money,$operation,$balance,$content,$type,$recognizable="1")
{
	$res=array(
		'user_id' => $uid,
		'money' => $money,
