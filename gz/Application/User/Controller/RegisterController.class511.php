...
class RegisterController extends UserbaseController {
    ...
    //  					M('User')->where("id=".$result)->setInc('balance',"10");
				// 		// setpoints($result,2,5,'元','151',0);
				// 		$balance = $this->user->where('id='.$result)->getField('balance');
				// 		financial_log($result,10,3,$balance,'被邀请注册',8);
				// 		setpoints($inviter_person['id'],1,10,'元','144',0);
				// 		$ba = $this->user->where('id='.$inviter_person['id'])->getField('balance');
				// 		financial_log($inviter_person['id'],10,3,$ba,'邀请注册',8);
				// 		if($invite_num == 10){
				...
				// 		setpoints($result,2,3,'元','151',0);
				// 		$score = $this->user->where('id='.$result)->getField('balance');
				// 		financial_log($result,3,3,$score,'被邀请注册',8);
				// 		setpoints($inviter_person['id'],2,3,'元','144',0);
				// 		$score1 = $this->user->where('id='.$inviter_person['id'])->getField('balance');
				// 		financial_log($inviter_person['id'],3,3,$score1,'邀请注册',8);
				// 		if($invite_num == 10){
				...
				//      	);
				// 	M('ScoreList')->add($data); 
				// 	M('User')->where("id=".$result)->setInc('balance',"3");
				// 	// setpoints($result,2,3,'元','154',0);
				// 	$score = $this->user->where('id='.$result)->getField('balance');
				// 	financial_log($result,3,3,$score,'新用户注册',8);
				// }
