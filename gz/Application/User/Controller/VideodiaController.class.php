...
class VideodiaController extends UserbaseController {
	public function _initialize() {
		...
		$this->videoconsultation = D('VideoConsultation');//视频咨询排版表
	...
	//电话咨询提交
	public function do_videodia()
	{
		
		$id = $this->uid;
		$member_id = I('post.member_id');
		$year = I('post.year');
		$video_date = I('post.video_date');
		...
		$video_date=strtotime($year.'-'.$video_date);
		unset($user_info);
		...
		$recount=$this->videodia->where("member_id=".$member_id." and patientmember_id=".$id." and status=1")->count();
		if($recount > 0)
		{
			unset($data);
			$data['code']=0;
			...
			if($result)
			{
				...
				if($result_sms['Code']=='OK')
				{
					...
					if($doctor_phone['deviceid']!='')
					{
						...
						$extra = array("type" => "5", "type_id" => $result,'user_type' => 1,'status' => 1);
	...
	public function lists()
	{
		...
		$result=$this->videodia->field("id,member_id,money,status,create_time")->where("patientmember_id=".$this->uid)->order("create_time desc")->limit($p.",".$limit)->select();
	...
	public function deal()
	{
		...
		$result=$this->videodia->field("id,member_id,name,age,phone,money,status,video_date,morn_after")->where("id=".$id)->find();
		if($result)
		{
			...
			$result['week']="星期".$weekarray[date("w",$result['video_date'])];
			$result['doctor_professional']=$res['professional'];
			$result['video_date']=date('Y年m月d日',$result['video_date']);
