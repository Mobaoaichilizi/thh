...
public	function do_login()
	{
		...
		if($result)
		{
			if($result['password']==sp_password($password))
			{
				...
				// $rest = $this->user->where("deviceid ='".$deviceid."' and username!='".$username."'")->select();
				// if($rest){
				...
				// 	}
				// }
				$this->user->where("id!=".$result['id']." and deviceid='".$deviceid."'")->save(array('is_login'=>1,'os' => '','deviceid' => ''));
