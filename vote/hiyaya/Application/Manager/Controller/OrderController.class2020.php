...
class OrderController extends ManagerbaseController {
    ...
    public function index()
	{
		...
		foreach($lock_room_list as &$val)
		{
			...
			//项目列表
			$val['project_info']=$this->ordersproject->where("shop_id=".$this->shopid." and order_id=".$val['order_info']['id'])->group("masseur_id")->select();
	...
	public function show_order()
	{
		...
		$result_project=$this->ordersproject->where("shop_id=".$this->shopid." and order_id=".$order_info['id'])->group("masseur_id")->order("down_time asc")->select();
	...
	public function one_up_clock()
	{
		...
		$order_project_list=$this->ordersproject->where("shop_id=".$this->shopid." and order_id=".$order_project_info['order_id']." and masseur_id=".$order_project_info['masseur_id'])->order("id desc")->select();
		foreach($order_project_list as $row)
		{
			$this->ordersproject->where("shop_id=".$this->shopid." and id=".$row['id'])->save(array('up_time' => time()));
		}
		if($order_project_info)
		{	
			$this->success("上钟成功！");		
	...
	public function one_down_clock()
	{
		...
		foreach($order_project_list as $row)
		{
			$this->ordersproject->where("shop_id=".$this->shopid." and id=".$row['id'])->save(array('down_time' => time()));
		}
	...
	public function post_update_master()
	{
		...
		$result=$this->ordersproject->where("id=".$project_id." and shop_id=".$this->shopid)->save(array('masseur_id' => $masseur_id,'scheduling_id' => $masseur_info['id']));
		if($result)
		{
