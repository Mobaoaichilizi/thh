	...
	public function get_order_addmasseura()
	{
		...
		*/
		...
		if($number > 1)
		{
			...
			//print_r($number_info);
			$is_show=$this->roundlog->where("shop_id=".$this->shopid." and createtime >=".$number_info['start_time'])->find();
			if(!$is_show)
			{
				unset($data_array);
				$data_array=array(
				...
				$show_info=$this->shopscheduling->where("start_time < ".time()." and end_time > ".time()." and shop_id=".$this->shopid." and (status=1 or status=2 or status=3)")->group("status")->order("end_time asc")->find();
				$show_list=$this->shopscheduling->where("start_time =".$show_info['start_time']." and end_time = ".$show_info['end_time']." and shop_id=".$this->shopid." and status=".$show_info['status'])->order("end_time asc")->select();
		...
		{
			/*
			$info[$key]['masseur_id']=$val['masseur_id'];
			$info[$key]['masseur_id']=$val['masseur_id'];
			$info[$key]['masseur_id']=$val['masseur_id'];
			$info[$key]['masseur_id']=$val['masseur_id'];
			$info[$key]['masseur_id']=$val['masseur_id'];
