...
class DataanalysisController extends ManagerbaseController {
	...
	public function _initialize() {
       ...
       $this->start_time1 = I('request.start_time');

       $this->end_time1 = I('request.end_time');
        if(!empty($this->start_time1)&&$this->end_time1=='')
        {
           // $this->start_time1=strtotime($this->start_time1);
            //$this->end_time1=strtotime($this->start_time1)+24*3600-1;
            $this->end_time1=mktime(0,0,0,date('m',strtotime($this->start_time1)),date('d',strtotime($this->start_time1))+1,date('Y',strtotime($this->start_time1)))-1;
            $this->start_time1=mktime(0,0,0,date('m',strtotime($this->start_time1)),date('d',strtotime($this->start_time1)),date('Y',strtotime($this->start_time1)));
            $this->assign("start_time",date('Y-m-d',$this->start_time1));
            $this->assign("end_time",date('Y-m-d',$this->end_time1));
        }
        else if($this->start_time1=='' || $this->end_time1=='')
        {
            //$this->start_time1=time();
            //$this->end_time1=time()+24*3600-1;

            $this->start_time1=mktime(0,0,0,date('m'),date('d'),date('Y'));
            $this->end_time1=mktime(0,0,0,date('m'),date('d')+1,date('Y'))-1;
        ...
        {
            $this->assign("start_time",$this->start_time1);
            $this->assign("end_time",$this->end_time1);
           $this->start_time1=strtotime($this->start_time1);
            $this->end_time1=strtotime($this->end_time1)+(24*3600)-1;
	...
	{
		$poor_day=round(($this->end_time1-$this->start_time1)/(24*3600));
		$redata=array();
		for($i=0;$i<$poor_day+1;$i++)
		{	
			$redata[$i]=date('Y-m-d',$this->start_time1+$i*3600*24);
			$redata_time[$i]=date('m-d',$this->start_time1+$i*3600*24);
			//新增订单收入
			unset($business);
        ...
        //订单消费统计
        $where=" status!=-1 AND shop_id=".$this->shopid." AND pay_time>=".$this->start_time1." AND pay_time<=".$this->end_time1;
        $ordermoney=$this->shoporders->where($where)->sum("pay_amount");
        $this->assign('ordermoney',$ordermoney);
        //售卡统计
        $where=" transaction_type!=1 AND type=1 AND  shop_id=".$this->shopid." AND createtime>=".$this->start_time1." AND createtime<=".$this->end_time1;
        $cardmoney = $this->shopfinancial->where($where)->sum('transaction_money');
        $this->assign('cardmoney',$cardmoney);
        //会员消费统计
        $where=" shop_id=".$this->shopid." AND  identity=1 AND type=2 AND createtime>=".$this->start_time1." AND createtime<=".$this->end_time1;
        $memberfee=$this->shopfinancial->where($where)->sum('transaction_money');
        $this->assign('memberfee',!empty($memberfee)?$memberfee*-1:'0');
        //新增会员数
        $where="shop_id=".$this->shopid." AND createtime>=".$this->start_time1." AND createtime<=".$this->end_time1;
        $addmembernum=$this->shopmember->where($where)->count();
        $this->assign('addmembernum',$addmembernum);
        //上客人数
        $where=" types!=3 AND is_del=0 AND  shop_id=".$this->shopid." AND createtime>=".$this->start_time1." AND createtime<=".$this->end_time1;
        $ordermembernum=$this->OrdersProject->where($where)->count();
        $this->assign('ordermembernum',$ordermembernum);
        //项目统计
        $where=" shop_id=".$this->shopid."  AND is_del=0 AND createtime>=".$this->start_time1." AND createtime<=".$this->end_time1;
        $projectmoney=$this->OrdersProject->where($where)->sum('pay_money');
        $this->assign('projectmoney',$projectmoney);
        //商品统计
        $productmoney=$this->OrdersProduct->where($where)->sum('pay_money');
        $this->assign('productmoney',$productmoney);
        //加钟人数
        $where=" types=3  AND is_del=0 AND  shop_id=".$this->shopid." AND createtime>=".$this->start_time1." AND createtime<=".$this->end_time1;
        $addpointnum=$this->OrdersProject->where($where)->count();
        //加钟率
        $addpointrate=number_format($addpointnum/$ordermembernum,2)*100;
        $this->assign('addpointrate',$addpointrate);
        //点钟人数
        $where=" types=2  AND is_del=0 AND  shop_id=".$this->shopid." AND createtime>=".$this->start_time1." AND createtime<=".$this->end_time1;
        $pointnum=$this->OrdersProject->where($where)->count();
        //点钟率
        $pointrate=number_format($pointnum/$ordermembernum,2)*100;
        $this->assign('pointrate',$pointrate);
    ...
    public function detailist()
    {
        ...
        if($act=='order')
        {
            ...
            $where=" status!=-1 AND shop_id=".$this->shopid." AND pay_time>=".$this->start_time1." AND pay_time<=".$this->end_time1;
            $list = $this->shoporders->where($where)->page($pagecur.','.$pagenum)->select();
         // echo   $this->shoporders->getLastsql();
         // die();
        ...
        else if($act=='card')
        {
            ...
            $where=" transaction_type!=1 AND type=1 AND  shop_id=".$this->shopid." AND createtime>=".$this->start_time1." AND createtime<=".$this->end_time1;
            $list = $this->shopfinancial->where($where)->page($pagecur.','.$pagenum)->select();
       ...
       else if($act=='addmember')
        {
            //新增会员明细
            $where="shop_id=".$this->shopid." AND createtime>=".$this->start_time1." AND createtime<=".$this->end_time1;
            $list=$this->shopmember->where($where)->page($pagecur.','.$pagenum)->select();
            foreach($list as $k=>$v)
            {
                $list[$k]['createtime']=date("y-m-d H:i:s",$v['createtime']);
            }
       ...
       else if($act=='member')
        {
            //上客人数明细
            $where=" types!=3   AND is_del=0 AND  shop_id=".$this->shopid." AND createtime>=".$this->start_time1." AND createtime<=".$this->end_time1;
            $list=$this->OrdersProject->where($where)->page($pagecur.','.$pagenum)->select();
       ...
       else if($act=='product')
       {
           //商品明细
           $where=" shop_id=".$this->shopid." AND createtime>=".$this->start_time1." AND createtime<=".$this->end_time1;
           $list=$this->OrdersProduct->where($where)->page($pagecur.','.$pagenum)->select();
           $arr=array("1"=>"轮钟","2"=>"点钟 ","3"=>"加钟");
           $payamount=0;
           $number=0;
           foreach($list as $k=>$v)
    ...
    public function importlist()
    {
        ...
        if($act=='order')
        {
            //订单明细
            $where=" status!=-1 AND shop_id=".$this->shopid." AND pay_time>=".$this->start_time1." AND pay_time<=".$this->end_time1;
            $list = $this->shoporders->where($where)->select();
            $arr=array("-1"=>"取消订单","1"=>"待支付","2"=>"已完成");
            $totalamount=0;
            $payamount=0;
            $invoicemoney=0;
            ...
            $countlist=array("id"=>'',"order_sn"=>'',"createtime"=>'',"status"=>'',"room_id"=>'合计',"total_amount"=>$totalamount,"invoice_money"=>$invoicemoney,"pay_time"=>"","pay_amount"=>$payamount);
            array_push($list,$countlist);
            $Title= $this->shop_name."订单明细";
            $Fieldsname=array( array('id','ID'),
                array('order_sn',"订单编号"),
                array('createtime',"开单时间"),
                array('status',"状态"),
                array('room_id',"包间"),
                array('total_amount',"订单原价"),
                array('invoice_money',"税费"),
                array('pay_time',"付款时间"),
                array('pay_amount',"已收金额"));

            exportExcel($Title,$Fieldsname,$list);

        }
        else if($act=='card')
        {
            $where=" transaction_type!=1 AND type=1 AND  shop_id=".$this->shopid." AND createtime>=".$this->start_time1." AND createtime<=".$this->end_time1;
            $list = $this->shopfinancial->where($where)->select();
            $arr=array("2"=>"次卡","3"=>"期限卡","4"=>"疗程卡");
            $transactionmoney=0;
            foreach($list as $k=>$v)
            {
            ...
            $countlist=array("id"=>'',"card_id"=>'',"transaction_type"=>'合计',"transaction_money"=>$transactionmoney,"createtime"=>'');
            array_push($list,$countlist);
            $Title= $this->shop_name."售卡明细";
            $Fieldsname=array( array('id','ID'),
                array('card_id',"卡名称"),
                array('transaction_type',"卡类型"),
                array('transaction_money',"金额"),
                array('createtime',"时间"));
            ...
            $Title= $this->shop_name."会员消费明细";
            $Fieldsname=array( array('id','ID'),
                array('project_id',"项目名称"),
                array('transaction_type',"类型"),
                array('transaction_money',"消费金额"),
                array('createtime',"消费时间"));
        ...
        else if($act=='addmember')
        {
            //新增会员明细
            $where="shop_id=".$this->shopid." AND createtime>=".$this->start_time1." AND createtime<=".$this->end_time1;
            $list=$this->shopmember->where($where)->select();
            foreach($list as $k=>$v)
            {
                $list[$k]['id']=$v['id'];
            ...
            $Title= $this->shop_name."新增会员明细";
            $Fieldsname=array( array('id','ID'),
                array('member_no',"会员编号"),
                array('member_name',"会员名称"),
                array('sex',"性别"),
                array('member_tel',"手机号码"));
        ...
        else if($act=='member')
        {
            //上客人数明细
            $where=" types!=3  AND is_del=0 AND  shop_id=".$this->shopid." AND createtime>=".$this->start_time1." AND createtime<=".$this->end_time1;
            $list=$this->OrdersProject->where($where)->select();
            $arr=array("1"=>"轮钟","2"=>"点钟 ","3"=>"加钟");
            ...
            $countlist=array("id"=>'',"title"=>'',"types"=>'合计',"pay_money"=>$payamount,"createtime"=>'');
            array_push($list,$countlist);
            $Title= $this->shop_name."上客人数明细";
            $Fieldsname=array( array('id','ID'),
                array('title',"项目名称"),
                array('types',"类型"),
                array('pay_money',"项目金额"),
                array('createtime',"时间"));
        ...
        else if($act=='product')
        {
            //商品明细
            $where=" shop_id=".$this->shopid." AND createtime>=".$this->start_time1." AND createtime<=".$this->end_time1;
            $list=$this->OrdersProduct->where($where)->select();
            $arr=array("1"=>"轮钟","2"=>"点钟 ","3"=>"加钟");
            $payamount=0;
            $number=0;
            foreach($list as $k=>$v)
            ...
            $countlist=array("id"=>'',"title"=>'',"unit_price"=>'合计',"number"=>$number,"total_price"=>$payamount,"createtime"=>'');
            array_push($list,$countlist);
            $Title= $this->shop_name."商品统计明细";
            $Fieldsname=array( array('id','ID'),
                array('title',"产品名称"),
                array('unit_price',"产品单价"),
                array('number',"产品数量"),
                array('total_price',"产品总价"),
                array('createtime',"时间"));
	...
	public function todaydaily()
	{
        ...
        $this->assign('cardmoney',$cardmoney);
        //会员消费统计
        $where=" shop_id=".$this->shopid." AND  identity=1 AND type=2 AND createtime>=".$start_time." AND createtime<=".($start_time+24*3600-1)."";
        $memberfee=$this->shopfinancial->where($where)->sum('transaction_money');
        $this->assign('memberfee',!empty($memberfee)?$memberfee*-1:'0');
        //新增会员数
        $where="shop_id=".$this->shopid." AND createtime>=".$start_time." AND createtime<=".($start_time+24*3600-1)."";
        $addmembernum=$this->shopmember->where($where)->count();
        $this->assign('addmembernum',$addmembernum);
        //上客人数
        $where=" types!=3  AND is_del=0 AND  shop_id=".$this->shopid." AND createtime>=".$start_time." AND createtime<=".($start_time+24*3600-1)."";
        $ordermembernum=$this->OrdersProject->where($where)->count();
        $this->assign('ordermembernum',$ordermembernum);
        //项目统计
        $where=" shop_id=".$this->shopid."   AND is_del=0 AND createtime>=".$start_time." AND createtime<=".($start_time+24*3600-1)."";
        $projectmoney=$this->OrdersProject->where($where)->sum('pay_money');
        $this->assign('projectmoney',$projectmoney);
        //商品统计
        $productmoney=$this->OrdersProduct->where($where)->sum('pay_money');
        ...
        $this->assign('addpointrate',$addpointrate);
        //点钟人数
        $where=" types=2   AND is_del=0 AND  shop_id=".$this->shopid." AND createtime>=".$start_time." AND createtime<=".($start_time+24*3600-1)."";
        $pointnum=$this->OrdersProject->where($where)->count();
        //点钟率
        $pointrate=number_format($pointnum/$ordermembernum,2)*100;
        $this->assign('pointrate',$pointrate);
		...
		$business_cash=$this->shoporders->where("shop_id=".$this->shopid." and ".$start_time." < pay_time  and pay_time <".($start_time+24*3600-1)." and pay_type=5")->sum('pay_amount');
		...
		$business_wechat=$this->shoporders->where("shop_id=".$this->shopid." and ".$start_time." < pay_time  and pay_time <".($start_time+24*3600-1)." and pay_type=4")->sum('pay_amount');
		...
		$business_alipay=$this->shoporders->where("shop_id=".$this->shopid." and ".$start_time." < pay_time  and pay_time <".($start_time+24*3600-1)." and pay_type=3")->sum('pay_amount');
		...
		$business_bank=$this->shoporders->where("shop_id=".$this->shopid." and ".$start_time." < pay_time  and pay_time <".($start_time+24*3600-1)." and pay_type=18")->sum('pay_amount');
...
//        $pagecur = !empty($pagecur)? $pagecur:1;
    ...
    public function productjson()
    {
        ...
        foreach ($datalist as $k=>$v)
        {
            ...
            $ordernum+=$datalist[$k]['ordernum'];
            $pay_money_single=M('OrdersProduct')->where(" product_id=".$v['id']." AND ".$sqlwhere)->sum('pay_money');
            $datalist[$k]['pay_money']=!empty($pay_money_single) ? $pay_money_single : 0;
            $pay_money+=$datalist[$k]['pay_money'];
            $total_price_single=M('OrdersProduct')->where(" product_id=".$v['id']." AND ".$sqlwhere)->sum('total_price');
            $datalist[$k]['total_price']=!empty($total_price_single) ? $total_price_single : 0;
            $total_price+=$datalist[$k]['total_price'];
            //税费计算
...
//        $pagecur = !empty($pagecur)? $pagecur:1;
	...
	public function individual(){
		...
		$poor_day=round(($end_time-$start_time)/(24*3600));
		//散客数量
		$orders_ids = $this->shoporders->where("shop_id=".$this->shopid." and member_id='' and status!=-1")->getfield('id',true);
		if(!empty($orders_ids)){
            $orders_ids = implode(',', $orders_ids);
			$member_count = $this->OrdersProject->where("shop_id=".$this->shopid." and createtime>".$start_time." and createtime<".$end_time." and is_del=0 and order_id in(".$orders_ids.")")->count();
		}else{
			$member_count=0;
		}
		...
		$this->assign("member_count",$member_count);
		//散客消费金额
		$member_balance=$this->shoporders->where("shop_id=".$this->shopid." and member_id='' and createtime>".$start_time." and createtime<".$end_time." and status=2")->sum('pay_amount');
		$member_balance=!empty($member_balance) ? $member_balance : 0;
		$this->assign("member_balance",$member_balance);
		//优惠金额
		$preferential=$this->shoporders->where("shop_id=".$this->shopid." and member_id='' and createtime>".$start_time." and createtime<".$end_time." and status=2")->sum("preferential_amount");
		$preferential=!empty($preferential) ? $preferential : 0;
		$this->assign("preferential",$preferential);
		//散客人均消费
        if(!empty($member_count)){
            $average_consumption = round($member_balance/$member_count, 2);
        }
        $average_consumption=!empty($average_consumption) ? $average_consumption : 0;
		...
		{	
			...
			unset($member_res);
			if(!empty($orders_ids)){
				$member_res= $this->OrdersProject->where("shop_id=".$this->shopid." and ".strtotime($redata[$i]).' < createtime  and createtime <'.(strtotime($redata[$i])+24*3600-1)." and is_del=0 and order_id in(".$orders_ids.")")->count();
			}else{
				$member_res = 0;
			}
			...
			//消费金额
			unset($consumption);
			$consumption=$this->shoporders->where("shop_id=".$this->shopid." and member_id='' and ".strtotime($redata[$i]).' < createtime  and createtime <'.(strtotime($redata[$i])+24*3600-1)." and status=2")->sum('pay_amount');
			$consumption_money[$i]=!empty($consumption) ? $consumption : 0;

			//优惠金额
			$preferential_res=$this->shoporders->where("shop_id=".$this->shopid." and member_id='' and ".strtotime($redata[$i]).' < createtime  and createtime <'.(strtotime($redata[$i])+24*3600-1)." and status=2")->sum("preferential_amount");
			$preferential_money[$i]=!empty($preferential_res) ? $preferential_res : 0;

			//轮钟数
			unset($loop_clock_res);
            if($orders_ids){
                $loop_clock_res = $this->OrdersProject->where("shop_id=".$this->shopid." and ".strtotime($redata[$i]).' < createtime  and createtime <'.(strtotime($redata[$i])+24*3600-1)." and is_del=0 and order_id in(".$orders_ids.") and types=1")->count();
            }
			$loop_clock_num[$i] = !empty($loop_clock_res) ? $loop_clock_res : 0;
			$loop_clock_total += $loop_clock_num[$i];

			//点钟数
			unset($point_clock_res);
            if($orders_ids){
                $point_clock_res = $this->OrdersProject->where("shop_id=".$this->shopid." and ".strtotime($redata[$i]).' < createtime  and createtime <'.(strtotime($redata[$i])+24*3600-1)." and is_del=0 and order_id in(".$orders_ids.") and types=2")->count();
            }
			$point_clock_num[$i] = !empty($point_clock_res) ? $point_clock_res : 0;
			$point_clock_total += $point_clock_num[$i];

			//加钟数
			unset($add_clock_res);
            if($orders_ids){
                $add_clock_res = $this->OrdersProject->where("shop_id=".$this->shopid." and ".strtotime($redata[$i]).' < createtime  and createtime <'.(strtotime($redata[$i])+24*3600-1)." and is_del=0 and order_id in(".$orders_ids.") and types=3")->count();
            }
			
			$add_clock_num[$i] = !empty($add_clock_res) ? $add_clock_res : 0;
			$add_clock_total += $add_clock_num[$i];

			//项目数量
			unset($project_res);
            if($orders_ids){
                $project_res = $this->OrdersProject->where("shop_id=".$this->shopid." and ".strtotime($redata[$i]).' < createtime  and createtime <'.(strtotime($redata[$i])+24*3600-1)." and is_del=0 and order_id in(".$orders_ids.")")->count();
            }
			
			$project_num[$i] = !empty($project_res) ? $project_res : 0;
			$project_total += $project_num[$i];

			//订单数量
			unset($order_res);
			$order_res = $this->shoporders->where("shop_id=".$this->shopid." and ".strtotime($redata[$i]).' < createtime  and createtime <'.(strtotime($redata[$i])+24*3600-1)." and status!=-1 and member_id=''")->count();
			$order_num[$i] = !empty($order_res) ? $order_res : 0;
			$order_total += $order_num[$i];
			
		}
		// dump($point_click_num);
		//表格统计数据
		$redata_array=$redata;
		$member_data_array=$member_data;
		...
		$this->assign("loop_clock_num",array_reverse($loop_clock_num));
		$this->assign("point_clock_num",array_reverse($point_clock_num));
		$this->assign("add_clock_num",array_reverse($add_clock_num));
		$this->assign("project_num",array_reverse($project_num));
		$this->assign("order_num",array_reverse($order_num));
		//折线图
		$redata=json_encode($redata);
		$member_data=json_encode($member_data);
		...
		$this->assign("loop_clock_total",$loop_clock_total);
		$this->assign("point_clock_total",$point_clock_total);
		$this->assign("add_clock_total",$add_clock_total);
    ...
    public function exportguest(){
        ...
        $orders_ids = implode(',', $orders_ids);
        if($orders_ids){
            $map = " and order_id in(".$orders_ids.")";
        }else{
            $map='';
        }
        for($i=0;$i<$poor_day+1;$i++)
        {   
            ...
            unset($member_res);
            if(!empty($orders_ids)){
                $member_res= $this->OrdersProject->where("shop_id=".$this->shopid." and ".strtotime($data[$i]['data'])." < createtime  and createtime < ".(strtotime($data[$i]['data'])+24*3600-1)." and is_del=0".$map)->count();
            }else{
                $member_res = 0;
            }
            
            $data[$i]['add_guest']=!empty($member_res) ? $member_res : 0;
            $add_guest += $data[$i]['add_guest'];
            
            //消费金额
            unset($consumption_res);
            $consumption_res=$this->shoporders->where("shop_id=".$this->shopid." and member_id='' and ".strtotime($data[$i]['data']).' < createtime  and createtime <'.(strtotime($data[$i]['data'])+24*3600-1)." and status=2")->sum('pay_amount');
            $data[$i]['consumption'] = !empty($consumption_res) ? $consumption_res : 0;
            $consumption += $data[$i]['consumption'];

            //优惠金额
            $preferential_res=$this->shoporders->where("shop_id=".$this->shopid." and member_id='' and ".strtotime($data[$i]['data']).' < createtime  and createtime <'.(strtotime($data[$i]['data'])+24*3600-1)." and status=2")->sum("preferential_amount");
            $data[$i]['preferential']=!empty($preferential_res) ? $preferential_res : 0;
            $preferential += $data[$i]['preferential'];


            //轮钟数
            unset($loop_clock_res);
            $loop_clock_res = $this->OrdersProject->where("shop_id=".$this->shopid." and ".strtotime($data[$i]['data']).' < createtime  and createtime <'.(strtotime($data[$i]['data'])+24*3600-1)." and is_del=0 and types=1".$map)->count();
            $data[$i]['loop_clock'] = !empty($loop_clock_res) ? $loop_clock_res : 0;
            $loop_clock_total += $data[$i]['loop_clock'];

            //点钟数
            unset($point_clock_res);
            $point_clock_res = $this->OrdersProject->where("shop_id=".$this->shopid." and ".strtotime($data[$i]['data']).' < createtime  and createtime <'.(strtotime($data[$i]['data'])+24*3600-1)." and is_del=0 and types=2".$map)->count();
            $data[$i]['point_clock'] = !empty($point_clock_res) ? $point_clock_res : 0;
            $point_clock_total += $data[$i]['point_clock'];

            //加钟数
            unset($add_clock_res);
            $add_clock_res = $this->OrdersProject->where("shop_id=".$this->shopid." and ".strtotime($data[$i]['data']).' < createtime  and createtime <'.(strtotime($data[$i]['data'])+24*3600-1)." and is_del=0 and types=3".$map)->count();
            $data[$i]['add_clock'] = !empty($add_clock_res) ? $add_clock_res : 0;
            $add_clock_total += $data[$i]['add_clock'];

            //项目数量
            unset($project_res);
            $project_res = $this->OrdersProject->where("shop_id=".$this->shopid." and ".strtotime($data[$i]['data']).' < createtime  and createtime <'.(strtotime($data[$i]['data'])+24*3600-1)." and is_del=0".$map)->count();
            $data[$i]['project_num'] = !empty($project_res) ? $project_res : 0;
            $project_total += $data[$i]['project_num'];

            //订单数量
            unset($order_res);
            $order_res = $this->shoporders->where("shop_id=".$this->shopid." and ".strtotime($data[$i]['data']).' < createtime  and createtime <'.(strtotime($data[$i]['data'])+24*3600-1)." and status!=-1 and member_id=''")->count();
            $data[$i]['order_num'] = !empty($order_res) ? $order_res : 0;
            $order_total += $data[$i]['order_num'];
            
        }

        $total=array('data'=>'合计','add_guest'=>$add_guest,'consumption'=>$consumption,'preferential'=>$preferential,'loop_clock'=>$loop_clock_total,'point_clock'=>$point_clock_total,'add_clock'=>$add_clock_total,'project_num'=>$project_total,'order_num'=>$order_total);
        ...
        $Fieldsname=array( array('data','时间'),
            ...
            array('loop_clock',"轮钟数"),
            array('point_clock',"点钟数"),
            array('add_clock',"加钟数"),
            array('project_num',"项目数"),
            array('order_num',"订单数"),
        );
