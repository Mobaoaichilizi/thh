...
class MasseurController extends MasseurbaseController
{
    ...
    public function lists()
    {
        ...
        // $count = $this->ordersproject->where($this->where.' and masseur_id='.$this->masseur_id.' and status=1')->order('createtime desc,id desc')->group('order_id')->count();
        $count = count($order);
...
//追加信息提交订单
    ...
    public function all_up()
    {
        ...
        if($order_info)
        {
            $result=$this->ordersproject->where("order_id=".$order_info['id']." and shop_id=".$this->shop_id." and up_time=0")->save(array('up_time' => time()));
            if($result)
            {
                unset($data);
                $data['code']=0;
    ...
    public function all_down()
    {
        ...
        if($order_info)
        {
            $orders_project=$this->ordersproject->where("order_id=".$order_info['id']." and shop_id=".$this->shop_id." and down_time=0")->select();
            ...
            $result=$this->ordersproject->where("order_id=".$order_info['id']." and shop_id=".$this->shop_id." and down_time=0")->save(array('down_time' => time()));
