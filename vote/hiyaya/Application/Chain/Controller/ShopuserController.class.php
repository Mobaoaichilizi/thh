...
class ShopuserController extends ChainbaseController {
	...
	public function get_index()
	{
		...
		foreach($users as &$row)
		{
			...
			$row['role_name'] = $row['role_id']==0 ? '本店超管' : '普通管理员';
		}
		unset($data);
		$data['code']=0;
		$data['msg']=$page;
	...
	public function addShopuser()
	{
		...
		{
			$shoplist=$this->shop->where('state=1 and chain_id='.$this->chainid)->order('id asc')->select();
