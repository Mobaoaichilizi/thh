...
class OrderController extends ApibaseController {
	...
	public function cate_project()
	{
		...
		{
			$val['project_list']=$this->shopitem->field("id,item_sn,item_name,item_price,item_duration,cover")->where("shop_id=".$this->shopid." and category_id=".$val['id'])->order("id desc")->select();
	...
	public function post_orders()
	{
		...
		$types=$_POST['types']; //轮钟类型  1：轮钟 2：点钟 3：加钟
		...
		if($result)
		{
			foreach($p_orders as $val)
			{
				if($val['p_types']==1)
				{
					...
					$masseur_info_level=$this->shopmasseur->where("id=".$val['masseur_id'])->getField("category_id");
					...
					$project_data=array(
						...
						'number' => $val['number'],
						'total_price' => ($val['number']*$project_info['item_price'])+($val['number']*$project_info['item_price']*$level_proportion*0.01),
					...
					$pay_total_money+=($val['number']*$project_info['item_price'])+($val['number']*$project_info['item_price']*$level_proportion*0.01);
	...
	public function go_pay()
	{
		...
		$order_info=$this->orders->field("id,order_sn,room_id,member_id,total_amount,pay_amount,status,createtime")->where("shop_id=".$this->shopid." and id=".$order_id)->find();
		//$this->assign('order_info',$order_info);
		...
		//$this->assign('is_one_uptime',$is_one_uptime);
		//获取订单产品
		$result_product=$this->ordersproduct->field("id,order_id,masseur_id,product_id,title,unit_price,number,total_price")->where("shop_id=".$this->shopid." and order_id=".$order_info['id'])->select();
		foreach($result_product as &$val)
		{
			if($val['masseur_id']!=0)
			{
	...
	{
		$sweep_list=$this->shoproom->field("id,room_name")->where("shop_id=".$this->shopid." and state=3")->order("id asc")->select();
		if($sweep_list)
		{
	...
	public function order_lists()
	{
		...
		foreach($result as &$val)
		{
			...
			if($val['member_id']!=0)
			{
				$val['member_info']=$this->shopmember->field("id,member_name,member_tel")->where("id=".$val['member_id'])->find();
			}else
			{
				$val['member_info']=array();
			}
			$val['masseur_list']=$this->ordersproject->field("id,order_id,masseur_id,title,up_time,down_time")->where("order_id=".$val['id'])->order("id desc")->select();
