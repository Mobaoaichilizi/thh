...
class QixianController extends ApibaseController
{
    ...
    public function index()
    {
        ...
        $list    = $this->ShopQixian->where($this->where)->order('id')->page($pagestart.','.$pagesize)->select();
        foreach ($list as $key=> $val)
        {
            ...
            $list[$key]['item_name']    = M('ShopItem')->where('id='.$val['item_id'])->getField('item_name');
            $list[$key]['item_price']   = M('ShopItem')->where('id='.$val['item_id'])->getField('item_price');
            $list[$key]['item_duration']= M('ShopItem')->where('id='.$val['item_id'])->getField('item_duration');
            $list[$key]['order_num']    = 0;
            if($val['state']==1)
            {
                $list[$key]['state']='启用';
            }
