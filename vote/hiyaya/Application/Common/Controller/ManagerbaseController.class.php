   ...
   function _initialize(){
		...
		if(!empty($session_user_id))
		{
			...
			if($shop_user['role_id'] == 0){
				...
				$menu_list= $node->where("parentid=0 and status=1 and id in (select node_id from dade_shop_access where role_id=".$shop_user['role_id']." and shop_id=".$shop_user['shop_id'].")")->order('listorder')->select(); 
			   foreach($menu_list as $n=> $val){	      
				  $menu_list[$n]['voo']=$node->where("parentid=".$val['id']." and status=1 and id in (select node_id from dade_shop_access where role_id=".$shop_user['role_id']." and shop_id=".$shop_user['shop_id'].")")->order('listorder')->select();  
				  foreach($menu_list[$n]['voo'] as $k=> $row)
				 {
					$menu_list[$n]['voo'][$k]['ttt']=$node->where("parentid=".$row['id']." and status=1 and id in (select node_id from dade_shop_access where role_id=".$shop_user['role_id']." and shop_id=".$shop_user['shop_id'].")")->order('listorder')->select();  
				 }		
			   }
			}
			$this->assign("shop_user",$shop_user);
			$this->assign("menu_list",$menu_list);
			$this->assign("shop_id",$shop_user['shop_id']);
			$this->assign("chain_id",$shop_user['chain_id']);
